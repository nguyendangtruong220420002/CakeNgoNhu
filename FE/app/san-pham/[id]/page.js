...
import { getProductById, getShopSettings } from '@/lib/api';
...
export default async function ProductDetailPage({ params }) {
  ...
  const [product, locale, settings] = await Promise.all([
    getProductById(id),
    getServerLocale(),
    getShopSettings(),
  ]);
  ...
  return (
    <>
      <main className="min-h-screen px-4 pt-3 pb-8 md:pt-8 md:pb-12 max-w-5xl mx-auto">
        <Link
          href="/san-pham"
      ...
      <IdeaCtaSection locale={locale} settings={settings} maxWidth="max-w-5xl" />
      <FeatureCardsSection locale={locale} maxWidth="max-w-5xl" />
